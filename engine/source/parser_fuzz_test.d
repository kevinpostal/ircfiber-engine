...
/// Records the outcome of a single fuzz check.
...
private string buildRandomIRCLine(ref Mt19937 rng, bool withTags,
    ...
    if (withPrefix) {
        ...
        const nick = nicks[uniform(0, nicks.length, rng)];
        const user = users[uniform(0, users.length, rng)];
        const host = hosts[uniform(0, hosts.length, rng)];
...
/// Runs the randomized fuzz suite through the parser.
...
/// Runs targeted adversarial-pattern fuzz cases.
void runTargetedFuzz() {
    ...
    threw = false;
    try parseIRCLinePublic("@time=2024;bot=1 :n PRIVMSG #c :hi", cfg);
